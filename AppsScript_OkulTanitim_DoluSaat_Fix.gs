...
function getTaken_(date, room) {
  date = formatDate_(date);
  room = normalizeRoom_(room);
  var rows = getRows_(SHEETS.BOOKINGS), out = [], seen = {};
  rows.forEach(function(r){
    var status = String(r[9] || 'Aktif').toLowerCase().trim();
    if (status === 'iptal' || status === 'cancelled' || status === 'pasif') return;
    var rowRoom = normalizeRoom_(r[6]);
    var rowDate = formatDate_(r[7]);
    var rowTime = formatTime_(r[8]);
    if (rowDate === date && rowRoom === room && rowTime) {
      var key = rowRoom + '-' + rowTime;
      if (!seen[key]) { out.push(key); seen[key] = true; }
    }
  });
  return out;
}
...
function bookAppointment_(data) {
  ...
  data.secilenOda = normalizeRoom_(data.secilenOda);
  ...
  var activeRooms = getRooms_().filter(function(r){ return r.active; }).map(function(r){ return normalizeRoom_(r.code); });
  if (activeRooms.indexOf(data.secilenOda) === -1) return { result:'error', message:'Seçilen görüşme birimi aktif değil.' };
  var slots = getSlots_();
  if (slots.indexOf(data.randevuSaati) === -1) return { result:'error', message:'Seçilen randevu saati aktif değil.' };
...
function appendBookingRow_(data) {
  ...
  var row = [new Date(), data.veliAdiSoyadi, data.ogrenciAdiSoyadi, data.ogrenciSinifi, data.telefon, data.veliEposta, data.secilenOda, data.randevuTarihi, data.randevuSaati, 'Aktif', data.kaynak || '', data.kampanya || '', data.reklamSeti || '', data.reklam || '', ''];
  var next = sh.getLastRow() + 1;
  // Tarih ve saat kolonlarını metin olarak tutuyoruz. Böylece 14:30 değerinin 14.30'a dönmesi yüzünden doluluk kontrolü bozulmaz.
  sh.getRange(next, 1, 1, row.length).setNumberFormat('@');
  sh.getRange(next, 1, 1, row.length).setValues([row]);
  sh.getRange(next, 1).setNumberFormat('dd.MM.yyyy HH:mm:ss');
  sh.getRange(next, 8).setNumberFormat('@');
...
function formatTime_(v) {
  ...
  if (m2) return ('0' + Number(m2[1])).slice(-2) + ':' + ('0' + Number(m2[2])).slice(-2);
  return s.length >= 5 ? s.substring(0,5) : s;
}
function normalizeRoom_(v) { return String(v || '').trim().toLowerCase(); }
